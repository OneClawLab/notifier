...
notifier 是一个 Linux 命令，既可作为普通 CLI 命令执行（执行完即退出），也可以 `start` 子命令启动常驻守护进程，`stop` 子命令停止守护进程。
...
│   ├── commands/             # 子命令实现
│   │   ├── start.ts          # start [--foreground]（启动 daemon）
│   │   ├── stop.ts           # stop（停止 daemon）
